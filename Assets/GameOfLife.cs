...
public class GameOfLife : MonoBehaviour
{
    ...
    {
        int count = 0;
		int[] neighbours = {(x - 1) * automationScale + y, (x + 1) * automationScale + y, 
							x * automationScale + (y - 1), x * automationScale + (y + 1),
							(x + 1) * automationScale + (y + 1), (x - 1) * automationScale + (y + 1),
							(x + 1) * automationScale + (y - 1), (x - 1) * automationScale + (y - 1)};
		
		for(int i=0; i<neighbours.Length; i++)
		{
			int n = neighbours[i];
			if(n > 0 && n < cells.Count)
			{
				if(cells[n].ifAlive)
					count++;
			}
